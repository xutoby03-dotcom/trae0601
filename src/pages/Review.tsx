...
const STATUS_COLORS: Record<string, string> = {
  ...
  reviewing: 'bg-coral-100 text-coral-600',
...
export default function Review() {
  ...
  const [reworkDeductionReasons, setReworkDeductionReasons] = useState<Record<string, string>>({})
  const [reworkDeductionAmounts, setReworkDeductionAmounts] = useState<Record<string, string>>({})
  const [removedReworkIds, setRemovedReworkIds] = useState<Set<string>>(new Set())
  ...
  const activeReworkItems = inspection.reworkItems.filter((ri) => !removedReworkIds.has(ri.id))
  ...
  const handleMarkRework = (checkItemId: string) => {
    const reason = reworkReasons[checkItemId] || ''
    const deductionReason = reworkDeductionReasons[checkItemId] || '清洁不彻底'
    const deductionAmount = parseFloat(reworkDeductionAmounts[checkItemId] || '0') || 0
    addReworkItem(inspection.id, checkItemId, reason, deductionReason, deductionAmount)
    setReworkReasons((prev) => {
      const next = { ...prev }
      delete next[checkItemId]
      return next
    })
    setReworkDeductionReasons((prev) => {
      const next = { ...prev }
      delete next[checkItemId]
      return next
    })
    setReworkDeductionAmounts((prev) => {
      const next = { ...prev }
      delete next[checkItemId]
      return next
    })
  ...
  const handleRemoveRework = (reworkItemId: string) => {
    setRemovedReworkIds((prev) => new Set(prev).add(reworkItemId))
  }
  ...
  const handleRework = () => {
    if (activeReworkItems.length === 0) return
    ...
    <div className="min-h-screen bg-warm-50 pb-28">
              ...
              {isExpanded && (
                <div className="border-t border-warm-50">
                  {items.map((item) => {
                    ...
                    const isReworked = inspection.reworkItems.some(
                      (ri) => ri.checkItemId === item.id && !removedReworkIds.has(ri.id)
                    )
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          {item.passed === false && (
                            <X className="w-4 h-4 text-coral-500 flex-shrink-0" />
                          )}
                          {item.passed === null && (
                          ...
                          {item.note && (
                            <span className="text-xs text-warm-400 truncate">({item.note})</span>
                          )}
                        </div>
        ...
        <div className="px-4 space-y-3">
          {activeReworkItems.map((ri) => {
            ...
            return (
              <div
                key={ri.id}
                className="bg-white rounded-xl border border-coral-200 p-4 space-y-3"
              >
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => handleRemoveRework(ri.id)}
                ...
                <div>
                  ...
                  <input
                    ...
                    value={ri.reason}
                    onChange={(e) =>
                      setReworkReasons((prev) => ({ ...prev, [ri.checkItemId]: e.target.value }))
                    }
                    placeholder="请输入返工原因"
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-warm-100 px-4 py-3 flex gap-3 z-20">
        ...
        {activeReworkItems.length > 0 && (
          ...
          >
            退回返工 ({activeReworkItems.length}项)
          </button>
        )}
      </div>
    </div>
