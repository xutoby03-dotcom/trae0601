...
export default function StatisticsPage() {
  ...
  const recentReturns = useMemo(() => {
    return [...returnRecords]
      ...
      .map((rr) => {
        const borrow = borrowRecords.find((b) => b.id === rr.borrowRecordId);
        const reservation = borrow ? reservations.find((r) => r.id === borrow.reservationId) : undefined;
        const ladder = reservation ? ladders.find((l) => l.id === reservation.ladderId) : undefined;
        return {
          returnRecord: rr,
          reservation,
          ladder,
      ...
      .filter((item) => item.reservation && item.ladder);
  }, [returnRecords, borrowRecords, reservations, ladders]);
  ...
  return (
    <div className="relative">
      <div className="animate-fade-in space-y-6 sm:space-y-8 max-w-6xl mx-auto">
        ...
        <div className="card p-5 sm:p-6">
          <CardHeader icon={RotateCcw} title="最近归还记录" desc="最新 10 条归还登记，含损坏与照片" accent="amber" />
          <div className="pt-4">
            {recentReturns.length === 0 ? (
              <div className="py-12 text-center">
                ...
                <p className="text-sm text-slate-500">暂无归还记录，借出去的梯子还没回来呢~</p>
              </div>
            ) : (
              <div className="space-y-3">
                {recentReturns.map(({ returnRecord, reservation, ladder }) => {
                  const configMap = {
                    [DamageLevel.NONE]: {
                      label: '无损坏',
                      icon: ShieldCheck,
                  ...
                  const config = configMap[returnRecord.damageLevel];
                  const DamageIcon = config.icon;

                  return (
                    <div
                      key={returnRecord.id}
                      className="p-4 rounded-2xl border border-slate-100 bg-gradient-to-r from-white to-slate-50 hover:shadow-md hover:border-amber-100 transition-all duration-200"
                    >
                      <div className="flex items-start justify-between gap-3 mb-3">
                        <div className="flex items-center gap-2 flex-wrap min-w-0">
                          <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${config.soft} ${config.border} border ${config.text}`}>
                            <DamageIcon className="w-3.5 h-3.5" />
                            {config.label}
                          </span>
                          <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs bg-slate-100 text-slate-600 font-medium">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="text-[11px] text-slate-400">借用人</div>
                            <div className="text-sm font-medium text-slate-700 truncate">
                              {reservation!.borrowerName} · {reservation!.building}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center gap-2.5 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="text-[11px] text-slate-400">梯子</div>
                            <div className="text-sm font-medium text-slate-700 truncate">
                        ...
                        <div className="flex items-center gap-2.5 min-w-0">
                          {returnRecord.damagePhoto ? (
                            <button
                              onClick={() => setPreviewPhoto(returnRecord.damagePhoto!)}
                              className="group relative flex items-center gap-2.5 min-w-0 w-full"
                            >
                              ...
                              <div className="min-w-0 flex-1 text-left">
                                ...
                                <div className="text-sm font-medium text-violet-600 group-hover:text-violet-700 transition-colors truncate">
                                  点击查看大图
                                </div>
                              </div>
                              <img
                                src={returnRecord.damagePhoto}
                                alt="损坏照片缩略图"
                                className="w-12 h-12 rounded-lg object-cover border border-slate-200 flex-shrink-0 group-hover:border-violet-300 group-hover:shadow-md transition-all duration-200"
                              />
                            </button>
                          ) : (
                            <div className="flex items-center gap-2.5 min-w-0">
                              ...
                              <div className="min-w-0">
                                <div className="text-[11px] text-slate-400">损坏照片</div>
                      ...
                      {returnRecord.damageDescription && (
                        <div className="mt-3 p-3 rounded-xl bg-slate-50/80 border border-slate-100">
                          <div className="text-[11px] text-slate-400 mb-1">损坏描述</div>
      ...
      {previewPhoto && (
        <div
          className="fixed inset-0 z-50 bg-slate-900/80 backdrop-blur-md animate-fade-in flex items-center justify-center p-4"
        ...
        >
          ...
          >
            <button
              onClick={() => setPreviewPhoto(null)}
              className="absolute -top-4 -right-4 w-10 h-10 rounded-full bg-white shadow-lg flex items-center justify-center text-slate-600 hover:text-slate-800 hover:bg-slate-100 transition-colors z-10"
            >
              <X className="w-5 h-5" />
            </button>
            <img
