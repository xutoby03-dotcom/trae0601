...
const CheckupList: React.FC = () => {
  ...
  }, [initialized, braces, searchParams, selectedBraces]);
  ...
  const handleSaveCheckupNote = () => {
    if (!currentBraces || !checkupDate) return;
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      {fromReminder && currentInventory && (
        <AlertBanner
          type="warning"
          title={`来自提醒：${currentBraces?.name}清洁片库存不足`}
          description="补充库存后提醒会自动消失"
          action={{
            label: '返回提醒中心',
      ...
      {currentInventory && currentInventory.currentStock <= currentInventory.lowStockThreshold && (
        <AlertBanner
          type="warning"
          title={`清洁片库存不足！仅剩 ${currentInventory.currentStock} 片`}
          description="请及时购买补充，避免影响清洁"
        />
      )}
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {inventories.filter(i => i.bracesId === selectedBraces).map(inventory => (
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="w-12 h-12 rounded-xl bg-warm-gray flex items-center justify-center hover:bg-gray-200 transition-colors"
                  >
                    <Minus size={20} />
                  </button>
                  <button
                ...
                <div className="p-3 rounded-xl bg-warm-gray/50">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <input
                      ...
                      onChange={(e) => setInventoryThreshold(selectedBraces, Number(e.target.value))}
                      className="w-16 input-base py-1 px-2 text-center"
                    />
          ...
          <CardContent className="space-y-4">
            {checkupIssues.length === 0 ? (
              ...
              <div className="space-y-4">
                {checkupIssues.map(issue => (
                  <div key={issue.category} className="p-4 rounded-xl bg-warm-gray/30">
