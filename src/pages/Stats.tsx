...
function isSameMonth(ts: number, year: number, month: number): boolean {
  const d = new Date(ts)
...
export default function Stats() {
  ...
  const monthLeaves = leaveSwaps.filter((l) => {
    ...
    return shift ? isSameMonth(shift.startTime, selectedYear, selectedMonth) : false
  })
  ...
  const hasData = monthShifts.length > 0 || monthLeaves.length > 0
    ...
    <div className="px-5 pt-14 pb-6">
      ...
      <div className="mt-4 flex items-center justify-between bg-stone-50 rounded-xl px-3 py-2.5">
        ...
        <div className="flex items-center gap-2">
          ...
          {!isCurrentMonth && (
            <button
              onClick={goToday}
              className="flex items-center gap-1 text-[11px] font-medium text-orange-500 bg-orange-50 px-2 py-1 rounded-full hover:bg-orange-100 transition-colors"
            >
              <RotateCcw size={11} />
              回到本月
            </button>
      ...
      {!hasData ? (
        <div className="flex flex-col items-center justify-center py-24 text-stone-400">
          ...
          <p className="mt-4 text-sm">{monthLabel}暂无数据</p>
        </div>
      ) : (
        <>
