...
export default function EditGear() {
  ...
  const [type, setType] = useState<GearType>(gear?.type || 'running_shoe')
  const [purchaseDate, setPurchaseDate] = useState(gear?.purchaseDate || '')
  const [price, setPrice] = useState(gear?.price?.toString() || '')
  const [usageFrequency, setUsageFrequency] = useState<UsageFrequency>(gear?.usageFrequency || 'weekly')
  const [maintenanceCycleDays, setMaintenanceCycleDays] = useState(gear?.maintenanceCycleDays?.toString() || '')
  const [maxUsage, setMaxUsage] = useState(gear?.maxUsage?.toString() || '')
  const [maxUsageUnit, setMaxUsageUnit] = useState<UsageUnit>(gear?.maxUsageUnit || 'km')
  const [photo, setPhoto] = useState(gear?.photo || '')
        ...
        <div className="rounded-2xl border border-white/[0.06] bg-white/[0.02] p-5">
          ...
          {photo ? (
            <div className="relative mb-3">
              <img src={photo} alt="装备照片" className="h-40 w-full rounded-xl object-cover" />
              <button
                type="button"
                onClick={() => setPhoto('')}
