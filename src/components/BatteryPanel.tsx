import { Battery as BatteryIcon, Plus, Trash2 } from 'lucide-react';
...
export default function BatteryPanel() {
  const { batteries, updateBattery, addBattery, removeBattery } = useFlightStore();
                ...
                <div className="mb-2 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className={`h-2 w-2 rounded-full ${
                      bat.status === 'critical' ? 'bg-[#FF4757] animate-pulse' : bat.status === 'warning' ? 'bg-[#FFB800]' : 'bg-[#00E5A0]'
                    }`} />
                    <span className="text-xs font-mono text-white/70">{bat.name}</span>
