...
import { BarChart3, CheckCircle, AlertTriangle, XCircle, ChevronDown, ChevronUp, Trophy, Target, Clock, Weight, RotateCcw, Filter, FilterX } from "lucide-react";
...
export default function SummaryPanel() {
  ...
  const [onlyUnready, setOnlyUnready] = useState(false);
  ...
  const displayedStudents = useMemo(() => {
    if (!onlyUnready) return studentSummaries;
    return studentSummaries.filter((s) => s.totalUnreadyKnots > 0);
  }, [studentSummaries, onlyUnready]);
  ...
  const totalUnreadyKnots = displayedStudents.reduce(
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg border border-olive-100 overflow-hidden flex flex-col h-full">
      ...
      <div className="p-4 border-b border-olive-100 bg-olive-50/30 space-y-3">
        ...
        <button
          onClick={() => {
            setOnlyUnready(!onlyUnready);
            setExpandedStudent(null);
            setExpandedKnot(null);
          }}
          className={`w-full flex items-center justify-between px-4 py-2.5 rounded-xl border-2 transition-all ${
            onlyUnready
              ? "bg-rope-50 border-rope-400 text-rope-700 shadow-sm shadow-rope-200/50"
              : "bg-white border-olive-200 text-olive-600 hover:border-olive-300 hover:bg-olive-50/50"
          }`}
        >
          ...
          <div
            className={`relative w-11 h-6 rounded-full transition-colors ${
              onlyUnready ? "bg-rope-500" : "bg-olive-300"
            }`}
          >
            <div
              className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow-md transition-transform ${
                onlyUnready ? "translate-x-5" : "translate-x-0.5"
              }`}
            />
          </div>
        </button>

        {onlyUnready && studentsWithUnready > 0 && (
          <div className="text-xs text-rope-600 text-center font-medium">
            共 {studentsWithUnready} 位学员有待加强的绳结
          </div>
        )}
      </div>
          ...
          <div className="flex flex-col items-center justify-center py-12 text-olive-400">
            {onlyUnready ? (
              <>
                ...
                <p className="text-sm font-medium text-emerald-600">所有学员都已达标！</p>
                <p className="text-xs mt-1">关闭筛选可查看全部学员</p>
              </>
            ) : (
              <>
