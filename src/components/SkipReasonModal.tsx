...
import { useCompletionStore, useLotteryStore } from '@/stores';
...
interface SkipReasonModalProps {
  open: boolean;
  onClose: () => void;
  onSkip: (skippedTaskId: string) => void;
}
...
export default function SkipReasonModal({ open, onClose, onSkip }: SkipReasonModalProps) {
  ...
  const addSkipRecord = useCompletionStore((s) => s.addSkipRecord);
  const currentTask = useLotteryStore((s) => s.currentTask);
  ...
  const handleConfirm = () => {
    ...
    const skippedId = currentTask.id;
    setSelected(null);
    setCustomText('');
    onSkip(skippedId);
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-[#FFF8F0] rounded-3xl p-6 w-full max-w-sm shadow-2xl border-2 border-[#FF6B35]/20 animate-in fade-in zoom-in-95 duration-150">
        <button
          onClick={handleClose}
        ...
        {selected === 'other' && (
          <textarea
            className="w-full h-[72px] resize-none rounded-xl border-2 border-gray-200 p-3 text-sm focus:border-[#4ECDC4] focus:outline-none transition-colors mb-4"
            placeholder="说说你的原因..."
            value={customText}
