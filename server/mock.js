const AREAS = ['A区研发部', 'B区产品部', 'C区市场部', 'D区行政部', 'E区会议室', 'F区休息区'];
const MODELS = ['冈村 Contessa 2', '海沃氏 Zody', 'Herman Miller Aeron', '西昊 C300', '黑白调 E3', '永艺 Mellet'];
const HANDLERS = ['李师傅', '王师傅', '赵师傅', '孙师傅'];
const armrestTypes = ['fixed', '3d', '4d', 'none'];
const faultTypes = ['noise', 'sinking', 'backrest_loose', 'wheel_jammed', 'armrest_broken', 'seat_collapse'];
const frequencies = ['rare', 'occasional', 'frequent', 'always'];
const statuses = ['pending', 'repairing', 'done', 'done', 'done', 'closed'];
...
function generateOrders(chairs) {
  const orders = [];
...
export function generateMockData() {
  ...
  const orders = generateOrders(chairs);
  return { chairs, orders };
}
